...
$.fn.player = function (debug) {
  ...
  var showTimes = function () {
    ...
    sliderPlayedEl.css("width", timePercent(audio.currentTime, audio.duration));

    // last time buffered
    var bufferEnd = 0;
    for (var i = 0; i < audio.buffered.length; ++i) {
      if (audio.buffered.end(i) > bufferEnd) bufferEnd = audio.buffered.end(i);
    }
    sliderLoadedEl.css("width", timePercent(bufferEnd, audio.duration));
  };
...
$(function () {
  ...
  var BeetsRouter = Backbone.Router.extend({
    ...
    itemQuery: function (query) {
      ...
      $.getJSON("item/query/" + queryURL, function (data) {
        var models = _.map(data["results"], function (d) {
          return new Item(d);
        });
  ...
  var ItemEntryView = Backbone.View.extend({
    ...
    setPlaying: function (val) {
      this.playing = val;
      if (val) this.$(".playing").show();
      else this.$(".playing").hide();
    },
  });
  ...
  var AppView = Backbone.View.extend({
    ...
    querySubmit: function (ev) {
      ev.preventDefault();
      router.navigate(
        "item/query/" + encodeURIComponent($("#query").val()),
        true,
      );
    },
    initialize: function () {
    ...
    audioPlay: function () {
      if (this.playingItem != null) this.playingItem.entryView.setPlaying(true);
    },
    audioEnded: function () {
      this.playingItem.entryView.setPlaying(false);
